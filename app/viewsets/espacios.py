"""
ViewSet para gestión de espacios de trabajo y horarios en contexto multi-tenant.

Acciones de Espacios:
- list: Listar espacios (filtrado según rol)
...
2. USUARIO/MECÁNICO/ADMINISTRATIVO/ALMACENERO: 
   - Solo pueden listar/consultar espacios (read-only)
   
3. Código único por empresa (espacios)
4. Auditoría en creación, edición, cambio de estado y cambio de activo
5. Validación de horarios: hora_inicio < hora_fin
"""
...
from app.models import EspacioTrabajo, HorarioEspacioTrabajo, Empresa
...
class EspaciosTrabajoViewSet(viewsets.ModelViewSet):
    """
    ViewSet para gestión de espacios de trabajo dentro de una empresa tenant.
    ...
    def get_queryset(self):
        ...
        Otros roles: también ven todos (pero con permisos de escritura restringidos)
        """
    ...
    def estado(self, request, pk=None, **kwargs):
        ...
        registrar_evento_desde_request(
            ...
            entidad_tipo="EspacioTrabajo",
            entidad_id=espacio.id,
            descripcion=f"Estado de espacio '{espacio.nombre}' cambió de {estado_anterior} a {espacio.estado}",
            metadata={
    ...
    def activo(self, request, pk=None, **kwargs):
        ...
        registrar_evento_desde_request(
            ...
            entidad_tipo="EspacioTrabajo",
            entidad_id=espacio.id,
            descripcion=f"Espacio '{espacio.nombre}' cambió de {'activo' if activo_anterior else 'inactivo'} a {'activo' if espacio.activo else 'inactivo'}",
            metadata={
    ...
    def editar_horario(self, request, pk=None, **kwargs):
        ...
        Parámetros query: ?horario_id={horario_id}
    ...
    def activo_horario(self, request, pk=None, **kwargs):
        ...
        # Auditoría: cambio de activo en horario
        motivo = request.data.get("motivo", "")
        dias = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"]
        dia_nombre = dias[horario.dia_semana]
        ...
        registrar_evento_desde_request(
            ...
            entidad_tipo="HorarioEspacioTrabajo",
            entidad_id=horario.id,
            descripcion=f"Horario {dia_nombre} cambió de {'activo' if activo_anterior else 'inactivo'} a {'activo' if horario.activo else 'inactivo'}",
            metadata={
                "activo_anterior": activo_anterior,
