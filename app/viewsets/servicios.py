"""ViewSet para gestión de catálogo de servicios en contexto multi-tenant.
- list: Listar servicios (filtrado según rol)
...
from app.models import ServicioCatalogo, Empresa
...
# VIEWSET DE SERVICIOS CATÁLOGO
class ServiciosCatalogoViewSet(viewsets.ModelViewSet):
    """ ViewSet para gestión del catálogo de servicios dentro de una empresa tenant.
    ...
    def get_queryset(self):
        ...
        USUARIO y MECÁNICO: solo ven activos """
        ...
        # Usuarios y técnicos solo ven servicios activos
        if rol_nombre in ["USUARIO", "MECÁNICO"]:
    ...
    def check_object_permissions(self, request, obj):
        """ Verificar permisos de objeto. USUARIO y MECÁNICO no pueden ver servicios inactivos."""
        if self.action == "retrieve":
            rol_nombre = request.user.rol.nombre if request.user.rol else None
            # Si es usuario o mecánico, solo pueden ver servicios activos
            if rol_nombre in ["USUARIO", "MECÁNICO"] and not obj.activo:
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        # Auditoría: servicio actualizado
    ...
    def estado(self, request, pk=None, **kwargs):
        ...
        serializer = self.get_serializer(servicio, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        # Guardar estado actual para auditoría
