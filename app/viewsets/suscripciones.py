...
1. CAMBIO DE PLAN (cambiar-plan → crear_payment_intent (accion=cambiar) → confirmar_pago (accion=cambiar))
2. RENOVACIÓN (crear_payment_intent (accion=renovar) → confirmar_pago (accion=renovar))
3. CANCELACIÓN DE CAMBIO PENDIENTE (cancelar-cambio)
...
from app.models import Suscripcion, Plan, Pago, Empresa, EstadoSuscripcion
from app.serializers.suscripciones import (
...
class SuscripcionViewSet(viewsets.ViewSet):
    ...
    1. CAMBIAR PLAN:
       - POST cambiar-plan → programa plan_pendiente
       - POST crear_payment_intent (accion='cambiar') → crea intent con monto calculado
       - Pago completado en Stripe
       - POST confirmar_pago (accion='cambiar') → registra pago, vincula a suscripción
       - Apply automático cuando llegue fecha_aplicacion_plan_pendiente  
    2. RENOVAR:
       - POST crear_payment_intent (accion='renovar') → crea intent con monto del plan actual
       - Pago completado en Stripe
       - POST confirmar_pago (accion='renovar') → extiende período actual
    3. CANCELAR:
       - POST cancelar-cambio → limpia plan_pendiente y pago asociado
    ENDPOINTS:
    - GET /api/{empresa_slug}/suscripciones/actual/ 
    - POST /api/{empresa_slug}/suscripciones/cambiar-plan/
    - POST /api/{empresa_slug}/suscripciones/cancelar-cambio/
    ...
    def get_empresa(self, request):
        """ Obtener la empresa del request con validación explícita de pertenencia.
        ...
        if hasattr(request, 'tenant') and request.tenant:
            ...
            if empresa_slug:
                ...
                return None
        # Validación explícita: el usuario debe pertenecer a esta empresa
    ...
    def _tiene_pago_confirmado_para_plan_pendiente(self, suscripcion):
        ...
        REGLA DE NEGOCIO:
        - Si el usuario ya pagó por un cambio pendiente, NO se puede reemplazar silenciosamente
        ...
        # Verificar si el pago está en estado COMPLETADO
    ...
    def _aplicar_plan_pendiente_si_corresponde(self, suscripcion, empresa):
        """ Verifica si existe un plan pendiente y su fecha de aplicación ya llegó.
        Si es así, lo aplica automáticamente dentro de una transacción.
        - Ejemplo: fin = 2025-03-31 23:59:59, inicio_siguiente = 2025-04-01 00:00:00
        Args:
            suscripcion: Objeto Suscripcion
            empresa: Objeto Empresa
        Returns:
            True si se aplicó el plan pendiente, False si no hay pendiente o fecha no llegó """
        if not suscripcion.plan_pendiente or not suscripcion.fecha_aplicacion_plan_pendiente:
            return False
        ahora = timezone.now()
        # Verificar si la fecha de aplicación ya llegó
        if ahora >= suscripcion.fecha_aplicacion_plan_pendiente:
            try:
                with transaction.atomic():
                    ...
                    empresa.save()
                    # Auditoría: plan pendiente aplicado (diferido con on_commit)
                    registrar_evento_on_commit(
                        empresa=empresa,
                        accion=AccionAuditoria.SUSCRIPCION_PLAN_PENDIENTE_APLICADO,
                        usuario=None,  # Aplicación automática, no hay usuario que lo dispare
                        entidad_tipo="Suscripcion",
                        entidad_id=suscripcion.id,
                        descripcion=f"Plan pendiente aplicado automáticamente: {plan_nuevo.nombre}",
                        metadata={
                ...
                logger = logging.getLogger(__name__)
                logger.error(
                    f"Error al aplicar plan pendiente para suscripción {suscripcion.id}: {str(e)}",
                    exc_info=True
                )
                return False
        return False
    ...
    def obtener_suscripcion_actual(self, request, empresa_slug=None):
        ...
        Responde 404 si:
        - Empresa no existe o Empresa no tiene suscripción activa """
        ...
        try:
            ...
            # Aplicar plan pendiente si su fecha ya llegó
    ...
    def cambiar_plan(self, request, empresa_slug=None):
        """ Programar cambio de plan para después del período actual de cobertura.
        POST /api/{empresa_slug}/suscripciones/cambiar-plan/
        Body:
        {
        ...
        Responde 404 si:
        Plan no existe o no está activo o Empresa no tiene suscripción. """
        empresa = self.get_empresa(request)
        if not empresa:
            return Response(
                {'error': 'Empresa no encontrada'},
        ...
        try:
            with transaction.atomic():
                ...
                # Si es el mismo plan: Error (debe usar renovación)
                if nuevo_plan.id == plan_actual.id:
                    return Response(
                        {'error': 'Para renovar el plan actual, usa la opción de renovación con pago.'},
                        status=status.HTTP_400_BAD_REQUEST
                    )
                    ...
                    return Response(
                        {
                            'error': f'No puedes cambiar de plan. Ya has confirmado pago para cambiar a {plan_pendiente_nombre}. '
                                    f'Espera a que se aplique automáticamente o contacta soporte para cancelarlo.',
                ...
                suscripcion.save()
                # Auditoría: cambio de plan programado
                registrar_evento_desde_request(
                    request,
                    empresa=empresa,
                    ...
                    entidad_tipo="Suscripcion",
                    entidad_id=suscripcion.id,
                    descripcion=f"Cambio de plan programado: {plan_actual.nombre} → {nuevo_plan.nombre} para el {fecha_aplicacion.isoformat()}",
                    metadata={
            ...
            return Response(
                {'error': 'Empresa sin suscripción activa'},
                status=status.HTTP_400_BAD_REQUEST
            )
        except Exception as e:
            return Response(
    ...
    def cancelar_cambio_pendiente(self, request, empresa_slug=None):
        ...
        try:
            with transaction.atomic():
                ...
                suscripcion.save()
                # Auditoría: cambio de plan cancelado
                registrar_evento_desde_request(
                    request,
                    empresa=request.tenant,
    ...
    def renovar_suscripcion(self, request, empresa_slug=None):
        """ DESHABILITADO: La renovación debe pasar por pago (crear_payment_intent + confirmar_pago).
        Flujo correcto:
        ...
        3. POST /api/{empresa_slug}/suscripciones/confirmar_pago/
           con accion='renovar'
        Esta ruta sigue existiendo pero retorna error explícito para claridad histórica. """
        return Response(
            {
                'error': 'La renovación de suscripción debe hacerse a través del flujo de pago. '
    ...
    def crear_payment_intent(self, request, empresa_slug=None):
        """ Crear un Payment Intent en Stripe para cambio o renovación de suscripción.
        VALIDACIÓN POR ACCIÓN:
        - accion='cambiar': Valida que hay suscripción, plan_pendiente programado, plan coherente
        ...
        try:
            ...
            if accion not in ['cambiar', 'renovar']:
                return Response(
                    {'error': f'accion inválida: "{accion}". Debe ser "cambiar" o "renovar"'},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if accion == 'cambiar':
                ...
                # Validar que el cambio esté programado correctamente
                if not suscripcion.plan_pendiente or suscripcion.plan_pendiente.id != plan.id:
                    return Response(
                        {
                            'error': 'Plan no está programado para cambio. '
                ...
                # En renovación, el plan DEBE ser el plan actual
                if plan.id != suscripcion.plan.id:
                    return Response(
                        {
                            'error': 'En renovación, el plan DEBE ser el plan actual. '
                                    'Si deseas cambiar de plan, usa accion=cambiar.',
                            'codigo_error': 'RENOVACION_PLAN_INCORRECTO'
                        },
                        status=status.HTTP_400_BAD_REQUEST
                    )
                descripcion = f"Renovación de plan {plan.nombre}"
            # Calcular monto SIEMPRE desde plan.precio_centavos (nunca del frontend)
            monto = plan.precio_centavos
            if monto <= 0:
                return Response(
                    {
                        'error': 'El plan tiene precio cero o inválido. Contacta soporte.',
    ...
    def confirmar_pago(self, request, empresa_slug=None):
        """ Confirmar pago en Stripe y materializar cambio o renovación.
        ...
        try:
            ...
            if settings.DEBUG and payment_intent_id.startswith('pi_dev_'):
                # En DEBUG: aceptar automáticamente
                payment_intent_status = 'succeeded'
            else:
                # En PRODUCCIÓN: validar en Stripe
            ...
            # Obtener el plan
            plan = Plan.objects.get(id=plan_id, activo=True)
            # Validar acción (ANTES de transacción)
            if accion not in ['cambiar', 'renovar']:
                return Response(
                    {'error': f'accion inválida: "{accion}". Debe ser "cambiar" o "renovar"'},
                    status=status.HTTP_400_BAD_REQUEST
            ...
            with transaction.atomic():
                ...
                if accion == 'cambiar':
                    ...
                    if not suscripcion.plan_pendiente or suscripcion.plan_pendiente.id != plan.id:
                        return Response(
                            {'error': 'Plan no está programado para cambio. Programa primero via cambiar_plan.'},
                            status=status.HTTP_400_BAD_REQUEST
                        )
                            ...
                            return Response(
                                {
                                    'error': 'Ya existe un pago confirmado para este cambio pendiente. '
                                            'No se permite múltiples pagos para el mismo cambio. '
                    ...
                    suscripcion.save()
                    # Auditoría: pago confirmado para cambio (diferido con on_commit)
                    registrar_evento_on_commit(
                        empresa=empresa,
                    ...
                    # IDEMPOTENCIA: Crear o reutilizar Pago para renovación
                    pago_para_usar = None
                    pago_fue_creado = False
                    try:
                        pago_para_usar, pago_fue_creado = Pago.objects.get_or_create(
                    ...
                    if not pago_fue_creado:
                        ...
                        if pago_para_usar.estado != 'COMPLETADO':
                            ...
                            pago_para_usar.processed_at = timezone.now()
                            pago_para_usar.save()
                    # Renovar: extender período actual
                    ...
                    empresa.save()
                    # Auditoría: suscripción renovada (diferido con on_commit)
                    registrar_evento_on_commit(
                        empresa=empresa,
                        ...
                        entidad_tipo="Suscripcion",
                        entidad_id=suscripcion.id,
                        descripcion=f"Suscripción renovada: plan {plan.nombre}",
                        metadata={
                            "plan": plan.nombre,
