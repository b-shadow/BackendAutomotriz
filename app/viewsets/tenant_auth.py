"""ViewSets para autenticación multi-tenant y resolución de tenants."""
...
from app.serializers.tenant_auth import (
...
def resolve_tenant(request):
    ...
    Resuelve un tenant por slug. Retorna datos básicos si existe. """
...
def tenant_register(request, tenant_slug):
    ...
    if serializer.is_valid():
        ...
        # Auditoría: usuario registrado en tenant
        registrar_evento_desde_request(
            request,
...
def tenant_login(request, tenant_slug):
    ...
    - Si juan@gmail.com existe SOLO en Empresa A:
      - Login en Empresa A con contraseña correcta => ✅ Permitido
    - Si juan@gmail.com existe en Empresa B (diferente):
      - Login en Empresa A => ❌ Rechazado (no existe en A)
    - Si juan@gmail.com existe en Empresa A con contraseña abc123:
      - Login con contraseña xyz789 => ❌ Rechazado
    Errores:
    - 400: Email o contraseña inválidos (genérico)
    - 400: Datos inválidos
    """
    ...
    if serializer.is_valid():
        ...
        # Auditoría: login exitoso en tenant
        registrar_evento_desde_request(
            request,
            empresa=tenant,
            ...
            entidad_tipo="Usuario",
            entidad_id=usuario.id,
            descripcion=f"Usuario {usuario.email} inició sesión en el tenant",
            metadata={
                "email": usuario.email,
                "rol": usuario.rol.nombre if usuario.rol else None,
                "origen": "tenant_login",
...
def tenant_logout(request, tenant_slug):
    ...
    Cierra la sesión del usuario actual revocando su sesión.
    ...
    # Revocar sesión: actualizar session_revoked_at
