"""ViewSet para gestión de usuarios en contexto multi-tenant.
Acciones:
...
class UsuariosViewSet(viewsets.ModelViewSet):
    """ ViewSet para gestión de usuarios dentro de una empresa tenant.
    ...
    def cambiar_rol(self, request, pk=None, **kwargs):
        ...
        registrar_evento_desde_request(
            ...
            entidad_tipo="Usuario",
            entidad_id=usuario.id,
            descripcion=f"Rol de {usuario.email} cambió de {rol_anterior} a {usuario.rol.nombre}",
            metadata={
    ...
    def cambiar_contrasena(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        # Auditoría: contraseña cambiada (sin exponer secretos)
        registrar_evento_desde_request(
            request,
            empresa=request.tenant,
            ...
            entidad_tipo="Usuario",
            entidad_id=usuario.id,
            descripcion=f"Contraseña de {usuario.email} fue actualizada",
            metadata={
    ...
    def preferencias_notificacion(self, request, *args, **kwargs):
        """Obtener o actualizar preferencias de notificación del usuario autenticado.
        ...
        if request.method == "GET":
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
            # Auditoría: preferencias de notificación actualizadas
            ...
            registrar_evento_desde_request(
                ...
                entidad_tipo="Usuario",
                entidad_id=usuario.id,
                descripcion=f"Preferencias de notificación de {usuario.email} fueron actualizadas",
                metadata=cambios
            )
            return Response(
                {
    ...
    def partial_update(self, request, pk=None, *args, **kwargs):
        ...
        - telefono"""
        usuario = self.get_object()
        # Validar que auto-edición o es ADMIN
    ...
    def eliminar(self, request, pk=None, **kwargs):
        ...
        # Auditoría: usuario eliminado (antes de borrar)
        registrar_evento_desde_request(
            request,
            empresa=request.tenant,
