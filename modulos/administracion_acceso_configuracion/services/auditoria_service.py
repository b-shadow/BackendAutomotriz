"""Servicio centralizado de auditorÃ­a para el sistema SaaS."""
import logging
import json
...
from modulos.administracion_acceso_configuracion.models import Auditoria, Empresa, Usuario
...
CAMPOS_SENSIBLES = {
    'password', 'passwd', 'pwd', 'contraseÃ±a',
...
CAMPOS_SENSIBLES_PARCIAL = {
    'hash', 'hashed',  # Hashes de contraseÃ±a
    'session', 'jwt',
    'oauth',
}
...
def normalizar_metadata(metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for clave, valor in metadata.items():
        ...
        if _es_campo_sensible(clave):
            logger.debug(f"Filtrando campo sensible en auditorÃ­a: {clave}")
            continue
...
# FUNCIONES PÃšBLICAS DE AUDITORÃA
...
def registrar_evento_auditoria(
    ...
    """ Registra un evento de auditorÃ­a en la base de datos. """
    try:
        # Validaciones bÃ¡sicas
        if not empresa or not isinstance(empresa, Empresa):
            logger.error(f"registrar_evento_auditoria: empresa invÃ¡lida o None")
            return None
        if not accion or not isinstance(accion, str):
            logger.error(f"registrar_evento_auditoria: accion vacÃ­a o invÃ¡lida")
            return None
        # Normalizar metadata
        ...
        logger.debug(f"Evento auditorÃ­a creado: {accion} en {empresa.slug}")
        return evento
    except Exception as e:
        logger.exception(f"Error registrando evento auditorÃ­a [{accion}]: {e}")
        return None
...
def registrar_evento_desde_request(
    ...
    """ Registra un evento de auditorÃ­a extrayendo IP y User-Agent del request. """
...
def registrar_evento_on_commit(
    ...
    """ Registra un evento de auditorÃ­a de forma diferida usando transaction.on_commit(). """
    def _registrar():
...
def construir_cambios(
    ...
    Retorna un diccionario con los campos que cambiaron, mostrando antes y despuÃ©s.
    Args:
...
class AccionAuditoria:
    # AutenticaciÃ³n Tenant
    USUARIO_REGISTRADO_TENANT = "Usuario registrado"
    USUARIO_LOGIN_TENANT = "Iniciar SesiÃ³n"
    USUARIO_LOGOUT_TENANT = "Cerrar SesiÃ³n"
    ...
    # Planes de VehÃ­culo
    PLAN_VEHICULO_CREADO = "Plan de vehÃ­culo creado"
    PLAN_VEHICULO_ACTUALIZADO = "Plan de vehÃ­culo actualizado"
    PLAN_VEHICULO_ESTADO_CAMBIADO = "Plan de vehÃ­culo, estado cambiado"
    
    # Detalles de Planes de VehÃ­culo
    PLAN_VEHICULO_DETALLE_CREADO = "Detalle de plan de vehÃ­culo creado"
    PLAN_VEHICULO_DETALLE_ACTUALIZADO = "Detalle de plan de vehÃ­culo actualizado"
    PLAN_VEHICULO_DETALLE_ESTADO_CAMBIADO = "Detalle de plan de vehÃ­culo, estado cambiado"
    PLAN_VEHICULO_DETALLE_ELIMINADO = "Detalle de plan de vehÃ­culo eliminado"
    ...
    # Recepciones de VehÃ­culos (CU16 SP1-T007)
    RECEPCION_REGISTRADA = "RecepciÃ³n de vehÃ­culo registrada"
    RECEPCION_ACTUALIZADA = "RecepciÃ³n de vehÃ­culo actualizada"
