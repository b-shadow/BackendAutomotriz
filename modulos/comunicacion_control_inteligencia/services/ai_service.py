...
- FINANZAS_VENTAS: Ventas presenciales en mostrador, emisión de facturas o recibos, consulta de caja y movimientos financieros.
...
"""
    ...
    def _get_finanzas_ventas_prompt(self) -> str:
        return f\"\"\"
EST�S EN EL M�DULO: FINANZAS Y VENTAS

Aqu� puedes registrar ventas presenciales de mostrador, emitir facturas/recibos a partir de pagos, y consultar el estado de la caja registradora.

ACCIONES PERMITIDAS Y SUS PAR�METROS:

1. AGREGAR_ITEM_VENTA: �sala para agregar un producto al carrito de venta presencial en mostrador.
   Par�metros:
   - "itemId": Nombre o c�digo del �tem a agregar.
   - "cantidad": Cantidad a vender.

2. EMITIR_FACTURA: �sala para emitir una factura o recibo.
   Par�metros:
   - "pago_taller": El pago asociado a esta factura.
   - "numero": (Opcional) N�mero del comprobante.
   - "nit_razon_social": NIT o Raz�n social del cliente a facturar.

3. CONSULTAR_CAJA: �sala cuando el usuario quiera saber el saldo actual, ingresos o egresos de su caja activa. (Solo informa, no requiere par�metros espec�ficos).

IMPORTANTE:
- Cuando el usuario indique una acci�n, si faltan par�metros, env�a la acci�n con status="PENDIENTE" y los campos que conozcas, llenando con "MANUAL" los faltantes.
- En el campo 'message', pide amigablemente los par�metros que faltan.
- Si el usuario ha dado todos los par�metros, env�a la acci�n con status="PENDIENTE" y pregunta si desea proceder/guardar.
- Solo env�a status="EJECUTADA" cuando el usuario haya dicho "s�", "guardar", "procede" despu�s de ver todos los datos.
\"\"\"
