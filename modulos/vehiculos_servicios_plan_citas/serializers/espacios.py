...
def _validar_solapamiento_horario(espacio_trabajo, dia_semana, hora_inicio, hora_fin, horario_id_excluir=None):
    ...
    - Ejemplo 2 (SÃ solapan): 08:00-13:00 y 12:00-16:00 (13:00 > 12:00 y 08:00 < 16:00)
    
    Args:
        espacio_trabajo: Instancia de EspacioTrabajo
        dia_semana: NÃºmero de dÃ­a (0-6, Lunes-Domingo)
        hora_inicio: time object de hora inicio
        hora_fin: time object de hora fin
        horario_id_excluir: ID del horario actual (si se estÃ¡ editando/reactivando), 
                            para excluirlo de la bÃºsqueda
...
class HorarioEspacioTrabajoListadoSerializer(serializers.ModelSerializer):
    ...
    Incluye dÃ­a de semana en formato legible.
    """
    dia_semana_display = serializers.SerializerMethodField()
    ...
    def get_dia_semana_display(self, obj):
        """Retorna nombre del dÃ­a de la semana."""
        dias = ["Lunes", "Martes", "MiÃ©rcoles", "Jueves", "Viernes", "SÃ¡bado", "Domingo"]
        return dias[obj.dia_semana]
...
class HorarioEspacioTrabajoCreacionSerializer(serializers.ModelSerializer):
    ...
    Validaciones:
    - dia_semana vÃ¡lido (0-6)
    - hora_inicio < hora_fin
    - NO solapamiento con otros horarios activos del mismo espacio/dÃ­a
    """
    ...
    def validate_dia_semana(self, value):
        """Validar que el dÃ­a sea vÃ¡lido (0-6)."""
        if value < 0 or value > 6:
            raise serializers.ValidationError(
                "El dÃ­a de la semana debe estar entre 0 (Lunes) y 6 (Domingo)."
            )
        return value
    ...
    def validate(self, data):
        ...
        con otros horarios activos del mismo espacio y dÃ­a.
        
        Un nuevo horario se crea ACTIVO por defecto, por lo que se valida
        contra todos los horarios activos del mismo espacio y dÃ­a.
        """
        ...
        if dia_semana is not None and hora_inicio and hora_fin:
            ...
            if espacio_trabajo:
                hay_conflicto, horario_conflictivo = _validar_solapamiento_horario(
                    ...
                    hora_fin=hora_fin,
                    horario_id_excluir=None  # Es creaciÃ³n, no hay ID
                )
    ...
    def create(self, validated_data):
        """Crear horario asignando empresa y espacio automÃ¡ticamente."""
...
class HorarioEspacioTrabajoEdicionSerializer(serializers.ModelSerializer):
    ...
    Validaciones:
    - hora_inicio < hora_fin
    - Si el horario estÃ¡ activo o serÃ¡ activado, valida NO solapamiento
    """
    ...
    def validate(self, data):
        ...
        La validaciÃ³n de solapamiento se aplica si:
        - El horario estÃ¡ activo actualmente, O
        - El horario SERÃ activado por esta ediciÃ³n
        """
        ...
        # Determinar si el horario estarÃ¡ activo despuÃ©s de la ediciÃ³n
...
class HorarioEspacioTrabajoActivoSerializer(serializers.ModelSerializer):
    ...
    El motivo NO se guarda en el modelo, solo se usa para auditorÃ­a.
    
    CRUCIALMENTE: Valida que al activar un horario, no se solape con
    otros horarios activos del mismo espacio y dÃ­a.
    
    Regla de negocio:
    - Puede haber horarios solapados registrados (mientras sean inactivos)
    - NO pueden existir dos horarios ACTIVOS solapados
    - Si se intenta activar un horario que se solapa con otro activo â†’ ERROR
    """
    ...
    motivo = serializers.CharField(
        ...
        allow_blank=True,
        write_only=True,
        help_text="Motivo del cambio de estado (opcional, solo para auditorÃ­a)"
    )
    ...
    def validate(self, data):
        ...
        - Si se pasa de activo a inactivo: PERMITIDO siempre (sin validaciÃ³n)
        ...
        if instance.activo == nuevo_estado:
            raise serializers.ValidationError(
                f"El horario ya estÃ¡ {'activo' if nuevo_estado else 'inactivo'}. "
        ...
        # No hay validaciÃ³n de solapamiento para desactivaciones
...
class EspacioTrabajoListadoSerializer(serializers.ModelSerializer):
    ...
    Incluye informaciÃ³n bÃ¡sica y estado del espacio.
    """
    ...
    def get_tipo_display(self, obj):
        """Retorna descripciÃ³n del tipo."""
        from modulos.vehiculos_servicios_plan_citas.models import TipoEspacioTrabajo
        tipo_map = dict(TipoEspacioTrabajo.choices)
        return tipo_map.get(obj.tipo, obj.tipo)
    
    def get_estado_display(self, obj):
        """Retorna descripciÃ³n del estado."""
        from modulos.vehiculos_servicios_plan_citas.models import EstadoEspacioTrabajo
...
class EspacioTrabajoDetalleSerializer(serializers.ModelSerializer):
    ...
    Incluye horarios relacionados y metadata de auditorÃ­a.
    """
...
class EspacioTrabajoCreacionSerializer(serializers.ModelSerializer):
    ...
    def validate_codigo(self, value):
        ...
        if empresa and EspacioTrabajo.objects.filter(
            ...
            raise serializers.ValidationError(
                f"Ya existe un espacio con cÃ³digo '{value}' en esta empresa."
            )
    ...
    def validate_tipo(self, value):
        ...
        if value not in dict(TipoEspacioTrabajo.choices):
            raise serializers.ValidationError(
                f"Tipo invÃ¡lido. Valores permitidos: {', '.join(dict(TipoEspacioTrabajo.choices).keys())}"
            )
        
        return value
    
    def create(self, validated_data):
        """Crear espacio asignando empresa automÃ¡ticamente."""
        empresa = self.context.get("empresa")
...
class EspacioTrabajoEstadoSerializer(serializers.ModelSerializer):
    ...
    Estados vÃ¡lidos: DISPONIBLE, OCUPADO, MANTENIMIENTO, TIEMPO_EXTENDIDO
    El motivo NO se guarda en el modelo, solo se usa para auditorÃ­a.
    """
    motivo = serializers.CharField(
        max_length=500,
    ...
    def validate_estado(self, value):
        ...
        if value not in estado_map:
            raise serializers.ValidationError(
                f"Estado invÃ¡lido. Valores permitidos: {', '.join(estado_map.keys())}"
            )
...
class EspacioTrabajoActivoSerializer(serializers.ModelSerializer):
    ...
    motivo = serializers.CharField(
        ...
        allow_blank=True,
        write_only=True,
        help_text="Motivo del cambio (opcional, solo para auditorÃ­a)"
    )
    ...
    def validate(self, data):
        ...
        if instance and instance.activo == nuevo_estado:
            raise serializers.ValidationError(
                f"El espacio ya estÃ¡ {'activo' if nuevo_estado else 'inactivo'}."
            )
